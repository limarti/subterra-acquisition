<template>
  <div class="h-full flex flex-col">
    <NavigationBar :showBackButton="true" @back="navigateBack">
      <template #left>
        <PageTitle :label="$t('projectList.title')" content="" />
      </template>
    </NavigationBar>

    <!-- Main content -->
    <OverlayScrollbarsComponent class="flex-1 px-6 py-4">
      <div>
        ...
        <div v-else-if="error" class="flex items-center justify-center py-12">
          <div class="text-center">
            ...
            <button class="px-4 py-2 bg-accent-info text-white rounded-full" @click="loadProjects">
              {{ $t('common.retry') }}
            </button>
          </div>
        </div>
        ...
        <div v-else class="grid gap-4 grid-cols-[repeat(auto-fill,minmax(8rem,1fr))]">
          <GridItem v-for="project in projects"
                    :key="project.id"
                    :item="project"
                    :isSelected="isItemSelected(project.id)"
                    :isMultiSelectMode="isMultiSelectMode"
                    @click="handleProjectClickWithNavigation"
                    @checkboxClick="handleCheckboxClick"
                    @longPress="handleItemLongPress">
            <template #icon>
              <svg class="w-10 h-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" xml:space="preserve">
                <g>
                  <g>
                    <path fill="var(--color-text-secondary)"
...
<script setup lang="ts">
  ...
  import GridItem from './aoi/GridItem.vue';
  ...
  import { ensureAreasExist } from './projectUtils';
  ...
  const handleProjectClickWithNavigation = (idProject: string) =>
  {
    handleItemClick(idProject, () =>
    {
  ...
  const handleCreateProject = async () =>
  {
    ...
    catch (err)
    {
      ...
      show(ToastNotification, { message: t('projectList.createError'), type: ToastType.ERROR });
    }
  };
